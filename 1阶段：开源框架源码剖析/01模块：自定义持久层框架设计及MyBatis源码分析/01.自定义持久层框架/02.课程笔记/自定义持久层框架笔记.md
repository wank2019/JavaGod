...
* **第三步：创建``Resources``类，用于读取客户端提供的配置文件**
...
* **第二步：创建数据库核心配置文件：``SqlMapConfig.xml``**
  ...
  在resources目录下创建如下``SqlMapConfig.xml``核心配置文件
...
* **第五步：测试读取``SqlMapConfig.xml``配置文件**
...
* **第一步：创建``SqlSessionFactoryBuilder``**
...
* **第二步：创建``XMLConfigBuilder``来解析``SqlMapConfig.xml**``
...
* **第三步：创建``XMLMapperBuilder类，来解析``*mapper.xml``**
...
* **第四步：完善``XMLConfigBuilder``类中``parseConfig``方法的第二步**
...
* **第五步：完善``SqlSessionFactoryBuild``类中的``build``方法中的第一步**

  这个时候我们以及把sqlMapConfig.xml以及*mapper.xml配置文件全部解析到了Configuration核心配置类中，

  接下来进入下一步：创建SqlSessionFactory接口及实现类DefaultSqlSessionFactory
